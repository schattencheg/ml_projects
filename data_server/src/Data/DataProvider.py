...
from src.Data.DataLoader import DataLoaderBase, DataLoaderCCXT
from src.Data.enums import DataPeriod, DataResolution
...
#region Data Processing
    ...
    @staticmethod
    def get_local_instruments(data_dir: str = 'data') -> Dict[str, List[str]]:
        """
        ...
        Returns:
            Dictionary with resolution as key and list of tickers as value
        """
        local_instruments = {}
        ...
        if os.path.exists(data_dir):
            for resolution_dir in os.listdir(data_dir):
                ...
                if os.path.isdir(resolution_path):
                    ...
                    # Extract ticker names (remove .csv extension)
                    tickers = [f[:-4] for f in csv_files]
                    
                    if tickers:
                        resolution = resolution_map.get(resolution_dir.lower(), resolution_dir)
                        local_instruments[resolution] = sorted(tickers)
        
        return local_instruments
    
    @staticmethod
    def get_all_local_tickers(data_dir: str = 'data') -> List[str]:
        """
        Get a unique list of all locally available tickers across all resolutions
        ...
        """
        local_instruments = DataProvider.get_local_instruments(data_dir)
        all_tickers = set()
        
        for tickers in local_instruments.values():
            all_tickers.update(tickers)
        
        return sorted(list(all_tickers))
    
    @staticmethod
    def check_local_data_exists(ticker: str, resolution: DataResolution, data_dir: str = 'data') -> bool:
        """
        Check if data exists locally for a specific ticker and resolution
    ...
    @staticmethod
    def get_local_data_info(ticker: str, resolution: DataResolution, data_dir: str = 'data') -> Optional[Dict]:
        """
        ...
        Returns:
            Dictionary with data info (rows, start_date, end_date, file_size) or None if not found
        """
        resolution_dir = os.path.join(data_dir, resolution.name)
        data_file = os.path.join(resolution_dir, f"{ticker}.csv")
